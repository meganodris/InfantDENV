...
transformed parameters{
  ...
  for(s in 1:4) for(i in 1:N_hi1) for(t in 1:Nt) pHI1[s,i,t] = 1 + log(exp((trueT0HI1[s,i]-1)*log(2)+log(10))*exp(-gamma[1,s]*ageHI1[i,t])/10)/log(2);
  for(s in 1:4) for(i in 1:N_hi2) for(t in 1:Nt) pHI2[s,i,t] = 1 + log(exp((trueT0HI2[s,i]-1)*log(2)+log(10))*exp(-gamma[2,s]*ageHI2[i,t])/10)/log(2);
  for(s in 1:4) for(i in 1:N_prnt) for(t in 1:Nt) pNT[s,i,t] = 1 + log(exp((trueT0NT[s,i]-1)*log(2)+log(10))*exp(-gamma[3,s]*ageNT[i,t])/10)/log(2);
...
model {
  ...
  //--- priors ---//
  sigma ~ normal(1,1);
  for(s in 1:4) to_vector(gamma[,s]) ~ normal(0,1);
...
generated quantities {
  ...
  // half-lives
  for(i in 1:3) for(s in 1:4) half_life[i,s] = log(2)/gamma[i,s];
  ...
  // serotype decay fit
  for(s in 1:4) for(t in 1:Ntfit) decay_fit_sero[1,s,t] = meanT0s[1,s]*exp(-gamma[1,s]*tfit[t]);
  for(s in 1:4) for(t in 1:Ntfit) decay_fit_sero[2,s,t] = meanT0s[2,s]*exp(-gamma[2,s]*tfit[t]);
  for(s in 1:4) for(t in 1:Ntfit) decay_fit_sero[3,s,t] = meanT0s[3,s]*exp(-gamma[3,s]*tfit[t]);
  
    // decay fit
  for(t in 1:Ntfit) decay_fit[1,t] = mean(decay_fit_sero[1,,t]);
  for(t in 1:Ntfit) decay_fit[2,t] = mean(decay_fit_sero[2,,t]);
  for(t in 1:Ntfit) decay_fit[3,t] = mean(decay_fit_sero[3,,t]);
