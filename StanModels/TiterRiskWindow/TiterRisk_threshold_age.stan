...
model {
  ...
  // likelihood
  for(i in 1:3) cases ~ poisson(probI[i,]*offset+0.001);
...
generated quantities {
  ...
  for(i in 1:3) for(t in 1:Nt) log_lik[i,t] = poisson_lpmf(cases[t] | probI[i,t]*offset+0.001);
  for(i in 1:3) predCases[i,] = poisson_rng(probI[i,]*offset+0.001);
